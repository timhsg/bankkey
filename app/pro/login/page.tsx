...
//  Layout 2 colonnes : panneau brandé navy + formulaire blanc
//  Google OAuth via Supabase + email/password en fallback
// ════════════════════════════════════════════════════════════════════════
...
function LoginInner() {
  ...
  async function handleGoogle() {
    ...
    // Sinon : Supabase fait window.location → on ne revient pas ici
  }
  ...
  const headline =
    isReset  ? 'Réinitialiser votre mot de passe' :
    isSignup ? 'Créer votre espace BankKey' :
               'Connexion à BankKey'

  const subtitle =
    isReset  ? 'Entrez votre email professionnel pour recevoir un lien sécurisé.' :
    isSignup ? 'Commencez votre essai gratuit de 30 jours. Aucune carte bancaire requise.' :
               'Accédez à votre espace courtier.'

  return (
    <div className="min-h-screen lg:grid lg:grid-cols-5 bg-slate-50">

      {/* ═══════ Colonne brandée (desktop) ═══════ */}
      <aside className="hidden lg:flex lg:col-span-2 relative overflow-hidden bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900 text-white p-12 flex-col justify-between">
        ...
        <div className="relative space-y-8">
          <h2 className="text-3xl font-semibold tracking-tight leading-tight">
            Répondez en premier
            <span className="block text-blue-200">à chaque demande de financement.</span>
          </h2>
          <p className="text-blue-100/80 leading-relaxed text-sm max-w-md">
            BankKey qualifie automatiquement chaque email de prospect, calcule un score de bancabilité et prépare votre réponse — en moins de 60 secondes.
          </p>
          ...
          <div className="space-y-3 pt-4">
            {[
              ...
              { icon: <LockIcon />,     text: 'Chiffrement TLS + AES-256, hébergement européen' },
              { icon: <GavelIcon />,    text: 'Conformité RGPD complète, isolation par cabinet' },
            ].map((item, i) => (
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 text-[11px] font-medium text-emerald-200 bg-emerald-500/10 border border-emerald-400/20 px-3 py-1.5 rounded-full">
            ...
            Programme pilote 2026 — Places limitées
          </div>
          <p className="text-[11px] text-blue-200/60 mt-4">
            © {new Date().getFullYear()} BankKey · Courtiers en crédit immobilier
          </p>
        </div>
      </aside>
      ...
      <main className="lg:col-span-3 flex flex-col min-h-screen lg:min-h-0">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-10 lg:py-16">
          <div className="w-full max-w-sm">
            ...
            {/* Tabs login/signup (pas en mode reset) */}
            {!isReset && (
            ...
            {/* Google OAuth (pas en mode reset) */}
            {!isReset && (
              <>
                <button
                  type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <div>
                ...
                <input
                  ...
                  autoComplete="email"
                  className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:border-transparent transition-colors"
                />
              </div>
...
function KeyIcon({ white }: { white?: boolean }) {
